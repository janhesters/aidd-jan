...
test.describe("Wikipedia Search", () => {
  test("given: a search term, should: render the matching article", async ({ page }) => {
...
import { setupOrganizationAndLoginAsMember, teardownOrganizationAndMember } from "../../utils";
...
test.describe("organization settings members page", () => {
  test("given: a logged out user, should: redirect to login page", async ({ page }) => {
  ...
  test("given: a user who is NOT a member, should: show 404 page", async ({ page }) => {
    const { organization, user } = await setupOrganizationAndLoginAsMember({ page });
...
test.describe("as Admin", () => {
  test("given: an admin, should: show invite cards and allow role changes", async ({ page }) => {
    const data = await setupMultipleMembers({
      otherMemberRoles: [OrganizationMembershipRole.member, OrganizationMembershipRole.owner],
      page,
